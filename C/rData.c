...
int main(){
    ...
    void tabela(unsigned char temperatura[32], unsigned char umidade[32], char aviso[200]) {
            system("clear");
    ...
    while(1){
        ...
        tabela(temperatura, umidade, aviso);
        ...
        //printf("dados c. %d \n", dados_compartilhado);
        //receivedData[0] = dados_compartilhado;
        //receivedData[1] = 0x0A;
        switch (receivedData[0])
        {
            case 0x07 :
                sprintf (aviso, "  Sensor 0x%02X funcionando normalmente\n", dados_compartilhado);
                tabela(temperatura, umidade, aviso); 
                sleep(temp);
            break;
            ...
            case 0x08 :
                if (dados_compartilhado <= 31) {
                    ...
                    sprintf (aviso, "  Sensor: 0x%02x \nMedida da umidade: %02d\n", dados_compartilhado, receivedData[1]);
                    tabela(temperatura, umidade, aviso);
                    sleep(temp);
                }
                else {
                    sprintf(aviso, "  Endereco nao encontrado\n");
                    tabela(temperatura, umidade, aviso);
                    sleep(temp);
                }
            break;
            ...
            case 0x0A :
                sprintf (aviso, "  Sensoriamento continuo de temperatura desativado\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;
            
            case 0x0B :
                sprintf (aviso, "  Sensoriamento continuo de umidade desativado\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;
            
            case 0x0C :
                sprintf (aviso, "  Comando nao existente\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;
            
            case 0x0D :
                sprintf (aviso, "  Endereco de sensor nao existe\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;

            case 0x0E :
                sprintf (aviso, "  A medicao de temperatura ja se encontra nesta situacao\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;

            case 0x0F :
                sprintf (aviso, "  A medicao de umidade ja se encontra nesta situacao\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;

            case 0x1F :
                sprintf (aviso, "  Sensor com problema\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;

            case 0xCF :
                sprintf (aviso, "  Protocolo nao idetificado!\n");
                tabela(temperatura, umidade, aviso);
                sleep(temp);
            break;
